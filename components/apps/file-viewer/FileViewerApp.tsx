...
import { findFile } from "@/core/files/file-system";
...
export function FileViewerApp() {
  ...
  const t = useT();
  const file = openFileId ? findFile(openFileId) : null;

  if (!file || file.action.type !== "text") {
    return (
  ...
  return (
    <div className="flex h-full flex-col bg-white text-black">
      <div className="flex shrink-0 items-center gap-2 border-b border-[#aca899] bg-[#ebf3fb] px-3 py-2">
        <FsIcon node={file} className="h-7 w-7" />
